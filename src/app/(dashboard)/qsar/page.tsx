...
export default function QsarPage() {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      const res = await fetch("/api/predictions", {
        ...
        body: JSON.stringify({ smiles, type: "qsar" }),
      });
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-xl font-bold">QSAR/QSPR-Modellierung</h1>
        <p className="text-sm text-muted-foreground mt-1">
          Quantitative Struktur-Aktivitäts- und Struktur-Eigenschafts-Beziehungen
        </p>
      </div>
      ...
      {result && (
        <div className="bg-card border border-border rounded-lg p-5">
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-semibold">QSAR/QSPR-Analyse</h2>
            <button
              onClick={() => {
                const w = window.open("", "_blank");
                w?.document.write(`<html><head><title>QSAR-Analyse</title><style>body{font-family:'IBM Plex Sans',sans-serif;padding:40px;max-width:800px;margin:0 auto}</style></head><body><h1>QSAR/QSPR-Analyse: ${smiles}</h1><pre style="white-space:pre-wrap">${result}</pre></body></html>`);
                w?.document.close();
                w?.print();
              }}
